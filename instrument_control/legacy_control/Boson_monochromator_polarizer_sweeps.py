...
startlam = 8.0 #starting wavelength [microns] - 8
stoplam = 13.0 #ending wavlength
incrlam = 0.05 #increment
lam = 0
lamCheck = 0
polarizerfl = 1 #polarizer in setup - 1; no polarizer - 0
polarizer_angle = 80 #degrees f
trialNum = '1'

if polarizerfl == 1:
    polstr = str(polarizer_angle)
    polstr2 = polstr.zfill(3) + 'deg'
else:
    polstr2 = 'NA_'
...
###CHANGES####
cam1 = Cam("COM4")#set to com 5
cam2 = Cam2("COM5")#set to com4
#####################
...
for x in range(0,steps): 
    ...
    #filter wheel should be in #4 position ("filter 4" the 8 micron cutoff filter)
    #"filter 6" -- no filter
    while(lam == lamCheck):
    ...
    lamCheck = np.copy(lam)
    
    if abs(float(lam) - float(curlam)) < 0.005: #make sure commanded and setting match
        print('setting correct')
    else:
        print('monochromator synch error')
    lamstr = str(lamnm)
    print("lamda=", lam)
    #print("curlam=", curlam)
    #print("Diff=", abs(float(lam) - float(curlam)))
...
#        fig.colorbar(cax)
        ...
        fig, ax = plt.subplots()
        lines = ax.plot(image16b_1[142,:])#set to 121
...
#        fig.colorbar(cax)
        ...
        fig, ax = plt.subplots()
        lines = ax.plot(image16b_2[142,:])#set to 127
  #      plt.ylim((22000,24000))
        ...
        im1.save(rootFileName +'\\' + fileName + '_CAM1_' + lamstr.zfill(5) + 'nm_' + '500C_' + 'angle' + polstr2 + trialNum + '.tiff')#str(n+1) + '_CAM1_' + '_' + lamstr.zfill(5) + 'nm_' + '1000C_' + 'angle' + polstr2 + '.tiff')
        
        im2 = Image.fromarray(image16b_2)
        im2.save(rootFileName + '\\' + fileName + '_CAM2_' + lamstr.zfill(5) + 'nm_' + '500C_' + 'angle' + polstr2 + trialNum + '.tiff')#str(n+1) + '_CAM2_' + '_' + lamstr.zfill(5) + 'nm_' + '1000C_' + 'angle' + polstr2 + '.tiff')
    ...
    np.savetxt(rootFileName + '\\' + fileName +'_CAM1_fpaTempCorr_degC_' + lamstr.zfill(5) + 'nm_' + 'angle' + polstr2,fpaTempCorr1)
    np.savetxt(rootFileName + '\\' + fileName +'_CAM2_fpaTempCorr_degC_' + lamstr.zfill(5) + 'nm_' + 'angle' + polstr2,fpaTempCorr2)
